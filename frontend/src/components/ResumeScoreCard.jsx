...
const ResumeScoreCard = ({ score, analysis, summary }) => {
    ...
    const { categories = {}, strengths = [], weaknesses = [], checks = [] } = analysis;
            ...
            <div className="grid md:grid-cols-3 gap-16 items-start">
                ...
                <div className="md:col-span-2 space-y-16 flex flex-col">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-12 pt-10 border-t border-white/10">
                        ...
                        <div className="group">
                            <div className="flex items-center space-x-4 mb-8">
                                <div className="p-3 bg-white/10 rounded-2xl backdrop-blur-md"><AlertTriangle className="w-6 h-6 text-white" /></div>
                                <h4 className="font-black uppercase text-sm tracking-[0.3em] text-white">Action Items</h4>
                            </div>
                            <ul className="space-y-8">
                                {weaknesses.slice(0, 3).map((w, i) => (
                                    <li key={i} className="flex flex-col text-lg text-white/60 font-medium leading-tight">
                                        <div className="flex items-start mb-2">
                                            <XCircle className="w-5 h-5 text-white mr-4 shrink-0 mt-1" />
                                            <span className="text-white font-black">{w.area}</span>
                                        </div>
                                        <p className="ml-9 text-sm opacity-50 italic">Fix: {w.fix}</p>
                                    </li>
                                ))}
                            </ul>
                        </div>
